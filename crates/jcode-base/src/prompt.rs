...
pub const SWARM_EFFORT_DIRECTIVE: &str = "# Swarm Effort\n\nYou are running at the maximum reasoning effort with swarm orchestration enabled. For any non-trivial task, decompose the work and use the `swarm` tool to spawn and coordinate parallel agents (spawn workers with concrete prompts, assign tasks, and collect their reports) instead of doing everything yourself in one thread. Prefer parallelizing independent subtasks across swarm members, and use a coordinator/plan when the work has multiple stages.\n\nWhen the user points you at a plan/spec document, asks for the entire plan, says not to stop until it is done, or asks for end-to-end delivery, this is a comprehensive mission: use `swarm task_graph` with `mode: \"deep\"` as the primary workflow, even though the selected effort is `swarm`. Seed every material section as a node, include implementation and verification nodes, and do not give a completion response until `swarm plan_status` shows the graph is terminal and its final adversarial gate has passed. For this workflow, run `swarm run_plan` with `background: false` unless the user explicitly asks for asynchronous execution. If workers are queued, stalled, or fail, inspect status and reassign, fix, or report the blocked node instead of silently continuing with a partial implementation. Never mark a plan or todo complete merely because tests pass for one slice. Only skip the swarm for trivial, single-step requests.";
